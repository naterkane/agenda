...
var Agenda = module.exports = function(config, cb) {
  ...
  } else if (config.elastic) {
    this.elastic(config.elastic.host, config.elastic.index, cb);
  }
...
Agenda.prototype.elastic = function(url, index, cb){
  this._url = url;
  this._client = new elasticsearch.Client({
    host: '192.168.99.100:9200', // url
    log: 'trace'
  }, cb);
  this.db_init();
  return this;
...
Agenda.prototype.db_init = function( collection, cb ){
  if (this._client){
    ...
    this._collection.indices.create({
      index: collection || 'agendajobs'
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    ...
    // Try an upsert.
    if (this._client){
      var id = this._collection.search({
        index: 'agendajobs',
        body: {
          query: {
            match: {name: props.name, type: 'single'}
          }
        },
        fields: 'id'
      })
      console.log("got elasticsearch id",id);
      this._collection.update({
        index: 'agendajobs',
        type: 'job',
        id: id,
